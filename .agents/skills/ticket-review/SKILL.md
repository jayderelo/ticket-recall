...
2. Identify and read exactly one target ticket. Read-only access is sufficient.
3. Determine the review evidence available: user-supplied context, linked source material, implementation changes, tests, pull requests, and current project conventions.
4. Evaluate the ticket for:
...
5. Report each finding with its category, impact, supporting evidence, and a concrete recommendation. Clearly distinguish facts from inferences.
6. State when the available evidence is insufficient to judge a section. Do not manufacture completeness.
